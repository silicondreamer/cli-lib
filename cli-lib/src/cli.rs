...
impl <'a> Printable for Command<'a> {
    fn print(&self) {
        println!("Printing command usage with id:{} and name {}",self.id,self.name)
    }
}
...
fn register_commands(commands : &mut HashMap<&str, Command>) {
    let c1 = Command {name:"test", id:0, callback:test};
    let c2 = Command {name:"exit", id:1, callback:exit};
